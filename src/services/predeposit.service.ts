...
export class PredepositService {
  ...
  async fetchFrontData(userAddress: AddressLike) {
    ...
    if (!userStatusRow) {
      ...
      isSigned = userStatusRow.signature
        ? verifyMessage(PREDEPOSIT_MESSAGE, userStatusRow.signature).toLowerCase() === userAddress.toString().toLowerCase()
        : false
    }
    const lpData: {
  ...
  async verifyAndStoreSignature(input: PredepositInput) {
    ...
    try {
      recoveredAddress = verifyMessage(PREDEPOSIT_MESSAGE, signature)
    } catch (err) {
